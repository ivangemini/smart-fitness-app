...
Status: source contract merged; runtime evidence collection active under Phase 14.

This matrix is the canonical checklist for P14-B Stories runtime completion. A scenario is complete only when its required environment was actually exercised. Source tests and CI do not substitute for physical-device or deployed-environment evidence.
...
- Do not activate external push as part of P14-B; P14-C owns real push delivery.
...
P14-B may be called runtime-complete only when all required rows have concrete evidence references or an explicitly accepted platform/environment exclusion. Production rollout remains a separate controlled action.
