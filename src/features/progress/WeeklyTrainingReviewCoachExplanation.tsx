...
const COPY = {
  en: {
    explain: 'Explain week with Coach',
    signIn: 'Sign in to ask Coach',
    answer: 'Coach explanation',
    evidence: 'Coach evidence',
    unavailable: 'Read-only weekly Coach explanations are not available on this server.',
    unsupported: 'Coach could not explain this weekly review from the supported question scope.',
    error: 'Coach could not explain this weekly review right now.',
    boundary:
      'Coach explains the already-derived weekly review only. It cannot recalculate the review or change/apply your program.',
  },
  ru: {
    explain: 'Объяснить неделю с Coach',
    signIn: 'Войти, чтобы спросить Coach',
    answer: 'Объяснение Coach',
    evidence: 'Данные Coach',
    unavailable: 'Read-only объяснение недели Coach недоступно на этом сервере.',
    unsupported: 'Coach не смог объяснить недельный обзор в поддерживаемой области вопросов.',
    error: 'Сейчас Coach не смог объяснить недельный обзор.',
    boundary:
      'Coach только объясняет уже рассчитанный недельный обзор. Он не может пересчитать его или изменить/применить программу.',
  },
} as const;

export function WeeklyTrainingReviewCoachExplanation({
  review,
}: {
  ...
  const question = useMemo(
    () => buildWeeklyTrainingReviewCoachQuestion({ locale, review }),
    [locale, review],
  );
  ...
  const explain = async () => {
    ...
    try {
      ...
      if (!supportsWeeklyTrainingReviewCoachExplanation(capabilities)) {
