...
export const getProgressHighlightsCopy = (locale: SupportedLocale) => ({
  ...
  periodAccessibility:
    locale === 'ru' ? 'Период основных сигналов' : 'Highlights analysis period',
  summary: locale === 'ru' ? 'Сводка сигналов' : 'Signal summary',
  records:
    locale === 'ru' ? 'Новые расчётные рекорды 1ПМ' : 'New estimated 1RM records',
  ...
  noItems:
    locale === 'ru' ? 'Нет сигналов этой категории.' : 'No signals in this category.',
  recordDetail: (value: string, unit: string, date: string) =>
    `${value} ${unit} e1RM · ${date}`,
  ...
  askCompanion:
    locale === 'ru' ? 'Обсудить сигналы с Companion' : 'Discuss signals with Companion',
  openStrength:
    locale === 'ru' ? 'Открыть силу и тренировки' : 'Open Strength & Training',
