...
export const getActivityProgressCopy = (locale: SupportedLocale) => ({
  ...
  periodAccessibility:
    locale === 'ru' ? 'Период активности' : 'Activity analysis period',
  summary: locale === 'ru' ? 'Сводка' : 'Summary',
  sessions: locale === 'ru' ? 'Тренировки' : 'Sessions',
  activeDays: locale === 'ru' ? 'Активные дни' : 'Active days',
  workoutsPerWeek:
    locale === 'ru' ? 'Тренировок в неделю' : 'Workouts per week',
  sessionsLast7Days:
    locale === 'ru' ? 'За последние 7 дней' : 'Last 7 days',
  ...
  askCoach:
    locale === 'ru'
      ? 'Обсудить активность с Companion'
      : 'Ask Companion about activity',
  openWorkoutHistory:
    locale === 'ru' ? 'Открыть всю историю' : 'Open full history',
